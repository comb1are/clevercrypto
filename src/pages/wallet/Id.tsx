            ...
            <div className="container px-4 flex flex-col flex-1 justify-between gap-3">
                <div className="w-full flex flex-col gap-4">
                    <div className="w-full flex flex-col gap-2">
                        ...
                        <div className="rounded-[18px] py-3 pr-2 pl-4 text-[15px] bg-(--intro-btn-bg) text-(--text-main)">Мой кошелек #1</div>
                    </div>
                    <div className="w-full flex flex-col gap-2">
                        <label className="text-(--grey) text-sm">Обзор кошелька</label>
                        <ul className="rounded-[18px] bg-(--intro-btn-bg) p-3 flex flex-col gap-4">
                            {WALLET_LIST.map((data) => (
                                <li key={data.id} className="w-full flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <img src={data.img} className="shrink-0 w-10 h-10 object-contain rounded-full" alt="" />
                                        <div>
                                            <h3 className="font-medium text-base text-(--text-main)">{data.title}</h3>
                                            <p className="text-sm text-(--grey)">{data.subtitle}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <h4 className="font-medium text-base text-(--text-main)">{data.amount}</h4>
                                        <p className="text-sm text-(--grey)">{data.price}</p>
                                    </div>
                                </li>
                            ))}
                            <li className="pt-2">
