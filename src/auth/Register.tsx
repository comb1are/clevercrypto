...
export default function Register() {
    ...
    const handleRegister = async () => {
        ...
        try {
            await register({ email: credential, password }).unwrap();
            navigate('/home', { replace: true });
        } catch (err: any) {
            const msg = err?.data?.detail ?? err?.data?.message ?? 'Ошибка регистрации';
...
function PasswordStep({ password, setPassword, onRegister, isLoading, validationError }: PasswordStepProps) {
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <input
                ...
                placeholder="Пароль"
                className="w-full h-12 px-4 bg-(--bg-secondary) border border-(--border-secondary) rounded-2xl text-(--main-dark) text-base tracking-[0.02em] outline-none caret-blue-500 placeholder-[#text-(--grey)]"
                onKeyDown={(e) => { if (e.key === 'Enter' && canSubmit) onRegister(); }}
            />
